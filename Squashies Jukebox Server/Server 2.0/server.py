...
@app.route('/PlayVLC')
def playVLC():
    songOnPlaying =  player.play()
    print('Playing "'+songOnPlaying+'"')
    socketio.emit('respond to sync', songOnPlaying)
    return 'play'
...
@socketio.on("sync")
def on_sync(data, methods=['GET', 'POST']):
    songs = player.playlist(True)
    print(data)
    print(songs[0]['@name'])
    emit("respond to sync", songs[0]['@name'])
